...
func SearchCharactersById(Id string) ([]byte, error) {
	...
	out := searchCharactersById(server, Id)
...
func searchCharactersById(server *http.Server, Id string) []byte {
	...
	jsData, err := charactersAccess(Id)
	if err != nil {
		panic(err)
		return nil
	}
...
func charactersAccess(Id string) (map[string]interface{}, error) {
	...
	apiURL := fmt.Sprintf("https://api.bgm.tv/v0/characters/%s", Id)
