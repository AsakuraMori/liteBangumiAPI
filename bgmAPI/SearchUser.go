...
func SearchUserByName(keyWord string) ([]byte, error) {
	...
	out := searchUserByName(server, keyWord)
...
func searchUserByName(server *http.Server, keyWord string) []byte {
...
func searchUser(keyWord string) (map[string]interface{}, error) {
	...
	apiURL := fmt.Sprintf("%s%s", "https://api.bgm.tv/v0/users/", encodedKeyword)
	// 创建 HTTP 客户端
	client := &http.Client{}
